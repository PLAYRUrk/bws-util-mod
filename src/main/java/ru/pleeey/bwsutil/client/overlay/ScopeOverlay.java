...
import ru.pleeey.bwsutil.client.aim.AimModule;
...
public final class ScopeOverlay {
    ...
    public static void tick(Minecraft mc, LocalPlayer player) {
        ...
        if (currentMode != ScopeMode.AUTO || !isDrawing) {
            // Выстрел произведён, натяжение отменено или режим сменился.
            AimModules.get().reset();
            return;
        }
        AimModules.get().tick(mc, player);
    }
    ...
    public static void render(GuiGraphics g, float partialTick) {
        ...
        if (currentMode == ScopeMode.AUTO) {
            ...
            if (isDrawing) {
                AimModules.get().renderLead(g, mc, player, new AimModule.LeadContext(
                    cx, cy, pixPerRad, zeroAngleDeg, charge, partialTick, color, outlineColor));
            }
        } else {
    ...
    private static void drawModeIndicator(GuiGraphics g, Minecraft mc,
        ...
        boolean isSteering = isAuto && AimModules.get().isSteering();
    ...
    private static int applyAlpha(int argb, float factor) {
        return ScopeGfx.applyAlpha(argb, factor);
    }

    private static int brighten(int argb) {
        return ScopeGfx.brighten(argb);
    }
}
